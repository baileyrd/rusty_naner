...
pub fn execute(_args: &[String]) -> i32 {
    ...
    let downloads_dir = naner_root
        .join(constants::directory_names::VENDOR)
        .join(constants::directory_names::DOWNLOADS);
    ...
    let installer =
        vendors::UnifiedVendorInstaller::new(&naner_root, essential_defs.clone(), &http);
    ...
    logger::success(&format!(
        "Repair scan complete. {repaired_count} vendor(s) restored."
    ));
    0
}
