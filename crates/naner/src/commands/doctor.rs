...
pub fn execute(args: &[String]) -> i32 {
    let porcelain = args.iter().any(|a| a.eq_ignore_ascii_case("--porcelain") || a.eq_ignore_ascii_case("-p"));
    ...
    let config_ok = config_path.as_ref().map_or(false, |p| config::load(&naner_root, Some(p)).is_ok());
    ...
    for (name, dir, installed) in &vendor_status {
        let (symbol, color) = if *installed { ("✓", "92") } else { ("✗", "91") };
        println!("\x1b[{color}m  {symbol} {name} (vendor/{dir})\x1b[0m");
    }
    logger::newline();

    let check_conflicts = args.iter().any(|a| a.eq_ignore_ascii_case("--conflicts") || a.eq_ignore_ascii_case("-c"));
                    ...
                    if let Ok(entries) = std::fs::read_dir(&dir) {
                        for entry in entries.flatten() {
                            ...
                            if p.is_file() {
                                ...
                                seen.entry(name).or_insert_with(Vec::new).push(p.display().to_string());
                            }
                        }
                    }
                }
            }
            let conflicts: Vec<_> = seen.into_iter().filter(|(_, paths)| paths.len() > 1).collect();
